...
run_bench() {
  local conc="$1"
  local run_no="$2"
  local tag="qwen_offload${OFFLOAD_GB}_conc${conc}_run${run_no}"

  echo "=== START QWEN: offload=${OFFLOAD_GB}, concurrency=${conc}, run=${run_no} ==="
...
echo "Alle Qwen-Benchmarks abgeschlossen."
